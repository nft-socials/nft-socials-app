...
trait IOnePostDaily<TContractState> {
    // Post creation
    fn create_daily_post(ref self: TContractState, content_hash: Array<felt252>) -> u256;
...
mod OnePostDaily {
    ...
    impl OnePostDailyImpl of IOnePostDaily<ContractState> {
        fn create_daily_post(ref self: ContractState, content_hash: Array<felt252>) -> u256 {
            ...
            // Create post - making all posts swappable for testing
            let post = Post {
                token_id,
                author: caller,
        ...
        fn token_uri(self: @ContractState, token_id: u256) -> ByteArray {
            ...
            // Reconstruct CID from array of felts
            let mut i = 0;
            while i < post.content_hash.len() {
                let chunk = format!("{:x}", *post.content_hash.at(i));
                ByteArrayTrait::append(ref uri, @chunk);
                i += 1;
            };
